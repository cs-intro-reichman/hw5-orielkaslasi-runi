public class Wordle {
	...
	In in = new In(filename);              // open the dictionary file
    String[] words = in.readAllStrings();  // read all words from file
    return words; 
    }
    ...
    for (int x = 0; x < secret.length(); x++) {
        ...
        if (guessChar == secret.charAt(x)) {
            resultRow[x] = 'G'; // exact match!
        } else if (containsChar(secret, guessChar)) {
            resultRow[x] = 'Y'; // somewhere here...
        } else {
            resultRow[x] = '_'; // nope
        }
    }
    ...
    public static boolean isAllGreen(char[] resultRow) {
		for (int i=0; i<resultRow.length; i++) {
            if (resultRow[i] != 'G') {
            return false;   // found something not G
        }
    }
    return true; //all G      
    }
